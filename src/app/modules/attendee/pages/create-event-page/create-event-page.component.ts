...
import {
  EVENT_TYPE,
  EVENT_FORM_FIELDS as FIELDS,
  MEETING_TYPE,
} from './../../constants/event-form.constant';
...
@Component({
  ...
  imports: [
    ...
    EventTimeZonePickerComponent,
    ReactiveFormsModule,
    CommonModule,
    ButtonComponent,
    RadioButtonComponent,
    SetPriceModalComponent,
    SetCapacityModalComponent,
...
export class CreateEventPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.eventFormService.flyer?.value) {
      this.flyerPreview = this.eventFormService.getImageSrc(
        this.eventFormService.flyer.value
      );
    }
  ...
  protected onToggle() {
    ...
    this.eventFormService.controlValueChanged(
      this.eventFormService.requireApproval
    );
  }
  ...
  private getErrorMessage(control: AbstractControl, fieldName: string): string {
    ...
    const label_ = (
      fieldName.split('.')?.[1] || fieldName.split('.')?.[0]
    ).toLowerCase();
  ...
  private parseObjectToFormdata(): FormData {
    ...
    const eventInfo: any = {
      ...
      event_type_id:
        this.backendEventTypes.find((v) => v.name === data[FIELDS.EVENT_TYPE])
          ?.id || 0,
      event_meeting_type_id:
        this.backendMeetingTypes.find(
          (v) => v.name === data[FIELDS.MEETING_TYPE]
        )?.id || 0,
      event_date: this.formatDateToDDMMYYYY(
        data[FIELDS.DATES]?.[0]?.[FIELDS.DATE]
      ),
      ...
      event_start_time_date: this.formatDateToDDMMYYYY(
        data[FIELDS.DATES]?.[0]?.[FIELDS.DATE]
      ),
      event_end_time_date: this.formatDateToDDMMYYYY(
        data[FIELDS.DATES]?.[1]?.[FIELDS.DATE]
      ),
      event_start_time: data[FIELDS.DATES]?.[0]?.[FIELDS.TIME] || '',
      event_end_time: data[FIELDS.DATES]?.[1]?.[FIELDS.TIME] || '',
      event_start_time_zone_id:
        data[FIELDS.DATES]?.[0]?.[FIELDS.TIME_ZONE] || '',
    ...
    if (
      data[FIELDS.MEETING_TYPE] === MEETING_TYPE.IN_PERSON &&
      data[FIELDS.IN_PERSON_DETAILS]
    ) {
    ...
    if (
      data[FIELDS.MEETING_TYPE] === MEETING_TYPE.VIRTUAL &&
      data[FIELDS.VIRTUAL_DETAILS]
    ) {
